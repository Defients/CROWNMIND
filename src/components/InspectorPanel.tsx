...
import {
  Shield, Heart, Swords, Coins, Compass, Hammer, Sparkles, Skull,
  Target, Zap, Lock, Eye, MapPin, TrendingUp,
} from 'lucide-react';
...
interface InspectorPanelProps {
  state: GameState;
  onCastSpell: (spellName: 'rallySpark' | 'zeeyaMend' | 'astryxFlare', targetX: number, targetY: number) => void;
  onPlaceManualBounty: (type: string, targetId: string, x: number, y: number, amount: number) => void;
  gameMode?: GameMode;
}

export default function InspectorPanel({ state, onCastSpell, onPlaceManualBounty, gameMode = 'coSovereign' }: InspectorPanelProps) {
  const selection = useUIStore((s) => s.selection);
  const canInteract = gameMode !== 'observer';

  if (!selection.entityId) {
    return (
      <Panel as="section" ariaLabel="Tactical inspector - no selection" glow="subtle" className="flex flex-col justify-center items-center p-6 text-center">
        ...
        <h3 className="text-xs font-semibold uppercase tracking-widest text-[#f5c84b]">Tactical Inspector</h3>
        <p className="text-[11px] text-[#eee8ff]/50 mt-2 max-w-[200px] leading-relaxed">
          Click any tile, hero, monster, building, or lair on the map to inspect details and issue tactical commands.
        </p>
        <div className="mt-4 flex flex-col gap-1.5 text-[10px] text-[#eee8ff]/40 font-mono">
          <span className="flex items-center gap-1.5"><Eye className="w-3 h-3" aria-hidden="true" /> Audit any entity</span>
          <span className="flex items-center gap-1.5"><Zap className="w-3 h-3" aria-hidden="true" /> Cast tactical spells</span>
          <span className="flex items-center gap-1.5"><Target className="w-3 h-3" aria-hidden="true" /> Commission bounties</span>
        </div>
      </Panel>
    );
  }
  ...
  if (!entity) {
    return (
      <Panel as="section" ariaLabel="Inspector - entity not found" glow="subtle" className="p-4 text-center">
  ...
  const monsterAI = entity.MonsterAI as import('../engine/Component').MonsterAIComponent | undefined;
  ...
  return (
    <Panel as="section" ariaLabel={`Inspector: ${entityName}`} glow="subtle" className="flex flex-col p-3 gap-3 overflow-hidden">
      <div className="flex items-start justify-between border-b border-[rgba(128,90,213,0.28)] pb-2">
        <div>
          ...
          <p className="text-[11px] text-[#eee8ff]/60 italic">
            {heroAI ? heroAI.heroClass : monsterAI ? 'Monster' : building ? building.buildingType : lair ? 'Lair' : 'Entity'}
          </p>
        </div>
      ...
      {health && (
        <ProgressMeter label="HP" value={health.hp} max={health.maxHp} showValue valueSuffix={`/${health.maxHp}`} />
      )}
      ...
      {combat && (
        <div className="grid grid-cols-3 gap-1.5">
          ...
          <div className="bg-[#07040d] border border-[rgba(128,90,213,0.18)] rounded-md p-1.5 text-center">
            ...
            <span className="font-mono font-bold text-[#9b5cff] text-sm">{combat.speed.toFixed(1)}</span>
          </div>
        </div>
      )}
      ...
      <div className="bg-[#07040d] border border-[rgba(128,90,213,0.18)] rounded-md p-2 text-xs flex items-center gap-1.5">
        ...
        <span className="font-mono text-[#eee8ff]/70">({x}, {y})</span>
      </div>

      {lair && (
        <div className="bg-gradient-to-r from-[#1a1028] to-[#07040d]/80 border-l-2 border-[#9b5cff] p-2.5 rounded-r-lg">
          <span className="text-[10px] font-mono uppercase text-[#f5c84b] font-bold block">Lair Threat</span>
      ...
      {!canInteract && (
        <div className="text-center py-4 flex flex-col items-center gap-2">
          ...
          <p className="text-[11px] text-[#eee8ff]/40 font-mono">Observer Mode — No interaction</p>
        </div>
      )}
    </Panel>
  );
